...
def tenerHijos():
    ...
    for persona in personas:
        if int(persona.edad) >= 21 and int(persona.edad) <= 40:
            if persona.estadoMarital.lower() == "married":
                #Encontrar los hijos de la persona
                hijosPersona = []
                for habitante in personas:
                    if habitante.padre == persona.id or habitante.madre == persona.id:
                        hijosPersona.append(habitante)

                #Comprobamos que la persona tenga menos de 5 hijos
                if len(hijosPersona) < 5:
                    genero = ["Male", "Female"]
                    genero = random.choice(genero)

                    nombre = fake.name()

                    apiRequest = requests.get(f"https://geneacity.life/API/createChildren/?name={nombre}&idInhabitant={persona.id}&gender={genero}&age=0")
                    
                    respueta = apiRequest.json()

                    if respueta['status'] == 1:
                        print("---------------------------------------")
                        print(f"{nombre} ha llegado al mundo! ")
...
def casar():
    ...
    for persona in personas:
        if persona.estadoMarital.lower() == "single" and int(persona.edad) >= 18 and int(persona.edad) <= 45:
            if persona.genero.lower() == "female":
                mujeresSolteras.append(persona)
            else:
                hombresSolteros.append(persona)
    
    
    parejas = []
    for i in range(min(len(mujeresSolteras), len(hombresSolteros))):
        parejas.append((mujeresSolteras[i], hombresSolteros[i]))
    
    for pareja in parejas:
        while True:
            x = random.randint(50, 9950)
            y = random.randint(50, 9950)
            
            traslapar = False
            for casa in infoCasas:
                if abs(x - casa["x"]) < 50 and abs(y - casa["y"]) < 50:
                    traslapar = True
                    break
        ...
        respuestaApi = requests.get(f"https://geneacity.life/API/createInhabitantUnion/?idInhabitant1={pareja[0].id}&idInhabitant2={pareja[1].id}&newHouseXPostition={x}&newHouseYPostition={y}")
        jsonRespueta = respuestaApi.json()
        
        if jsonRespueta["status"] == 1:
            print(f"Casados ->{pareja[0].nombre} {pareja[1].nombre}: ({x}, {y})")
        """else:
            print(f"Error al casarse-> {pareja[0].nombre} {pareja[1].nombre}: ({x}, {y})")
            print(jsonRespueta["error"])"""
...
def ejecutarCiclo():
    ...
    def contarCiclos():
        with open("./archivosJuego/horas.txt", "r") as archivo:
            ultimaConexion = datetime.datetime.fromisoformat(archivo.read().strip())

        horaActual= datetime.datetime.now()
        diferencia = horaActual - ultimaConexion
    
        ciclosPasados = int(diferencia.total_seconds() // (3 * 60 * 60))
        print(f"Horas pasadas: {diferencia}")
        print(f"Ciclos pasados: {ciclosPasados}")

        for iCiclo in range(ciclosPasados):
            casar()
            tenerHijos()
...
def iniciarJuego():
    ...
    if len(personas) > 0:
        objJugador = encontrarObjetoPersona(datosJuego["id"])

    #Si no se ha seleccionado un personaje para jugar
    if datosJuego["id"] == None or objJugador == False:
    ...
    while running:
            ...
            #Datos jugador.
            infoJugador = f'{objJugador.nombre} {objJugador.edad}'
            textoSuperficie = fuente.render(infoJugador, True, (255, 255, 255))
            vtnJuego.blit(textoSuperficie, (10, 60))
